...
void AnalyticsDashboard::drawBoxes(QWidget* scrollAreaWidgetContents, System* system, QScrollArea* scrollArea, QStackedWidget* ListComponents) {
	...
	if (properties.size() != 0) {
		for (auto it = properties.begin(); it != properties.end(); ++it) {
			...
			QLabel* AddressLine = new QLabel(propertyCard);
			AddressLine->setObjectName("apartment" + i);
			AddressLine->setGeometry(QRect(400, 20, 231, 21));
			string strApartment = "Address Line : " + (*it)->GetAddressLine();
			AddressLine->setText(strApartment.c_str());
			...
			City->setObjectName("buildingNo" + i);
			City->setGeometry(QRect(400, 140, 191, 21));
			string strBuilding = "City : " + (*it)->GetCity();
			City->setText(strBuilding.c_str());
